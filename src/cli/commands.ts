...
import { loadConfig, generateConfigFile } from '../config/loader';
import { runPipeline, runAnalysisOnly } from '../core/pipeline';
...
program
  ...
  .action(async (options) => {
    ...
    // Create config file
    const configPath = generateConfigFile(projectRoot);
...
program
  ...
  .action(async (dir, options) => {
    ...
    const analysis = await runAnalysisOnly(config);
...
program
  ...
  .action(async (dir, options) => {
    ...
    const overrides: Partial<DocgenConfig> = {
    ...
    if (options.only) {
      overrides.generators = options.only.split(',').map((g: string) => g.trim()) as any;
    }
...
program
  ...
  .action(async (dir, options) => {
    ...
    const overrides: Partial<DocgenConfig> = { outputDir: options.output };
    if (options.ai === false) {
      overrides.ai = { enabled: false, provider: '', model: '', maxTokens: 0, temperature: 0 };
    }
    const config = loadConfig(projectRoot, overrides);
    ...
    const watchDirs = ['src', 'lib', 'app', 'routes', 'api'].map(d =>
  ...
  .action(async (dir, options) => {
    ...
    try {
      ...
      app.get('*', (req: any, res: any) => {
        ...
        if (!reqPath.endsWith('.md')) reqPath += '.md';
