...
import { summarizeProject } from '../../ai/reasoner';
import { hashContent } from '../../utils/helpers';
import { logger } from '../../utils/logger';

const log = logger.child('ReadmeGen');
...
export async function generateReadme(
  ...
  const overview = await summarizeProject(analysis, config.ai);
  ...
  if (hasNode) sections.push('- Node.js >= 18.0.0');
  if (hasPython) sections.push('- Python >= 3.8');
  sections.push('');
  ...
  if (testDeps.length > 0) {
    ...
    sections.push('```bash');
    if (hasNode) sections.push('npm test');
    if (hasPython) sections.push('pytest');
    sections.push('```\n');
  }
  ...
  return {
    ...
    inputHash: hashContent(JSON.stringify(analysis.name + analysis.files.length)),
  };
}
