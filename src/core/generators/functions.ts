...
import { ProjectAnalysis, DocgenConfig, GeneratedDoc, FunctionInfo, ClassInfo } from '../../types/definitions';
import { staticFunctionSummary } from '../../ai/reasoner';
...
export async function generateFunctionDocs(
  ...
  for (const file of analysis.files) {
    ...
    if (exportedFuncs.length === 0 && exportedClasses.length === 0) continue;
...
function formatFunctionDoc(func: FunctionInfo): string {
  const lines: string[] = [];
  ...
  if (func.existingDoc) {
    ...
    lines.push(`${staticFunctionSummary(func)}\n`);
  }
...
function formatClassDoc(cls: ClassInfo): string {
  const lines: string[] = [];

  lines.push(`### \`class ${cls.name}\`\n`);
  if (cls.superClass) lines.push(`**Extends:** \`${cls.superClass}\`\n`);
  if (cls.implements.length > 0) lines.push(`**Implements:** ${cls.implements.map(i => `\`${i}\``).join(', ')}\n`);
  if (cls.decorators.length > 0) lines.push(`**Decorators:** ${cls.decorators.join(', ')}\n`);
  if (cls.existingDoc) lines.push(`${cleanDoc(cls.existingDoc)}\n`);
