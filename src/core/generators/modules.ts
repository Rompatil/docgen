...
import { summarizeModule, staticModuleSummary, staticFunctionSummary } from '../../ai/reasoner';
import { hashContent } from '../../utils/helpers';
import { logger } from '../../utils/logger';

const log = logger.child('ModuleGen');
...
export async function generateModuleDocs(
  ...
  for (const module of analysis.modules) {
    if (module.files.length < 1) continue; // Skip empty modules
...
function generateModuleIndex(analysis: ProjectAnalysis): GeneratedDoc {
  ...
  return {
    ...
    inputHash: hashContent(analysis.modules.map(m => m.name).join(',')),
  };
}

async function generateSingleModuleDoc(
  module: ModuleInfo,
  config: DocgenConfig
): Promise<GeneratedDoc> {
  const sections: string[] = [];
  ...
  const description = config.ai.enabled
    ? await summarizeModule(module, config.ai)
    : staticModuleSummary(module);
  ...
  for (const file of module.files) {
    ...
    const meta: string[] = [];
    if (funcCount > 0) meta.push(`${funcCount} functions`);
    if (classCount > 0) meta.push(`${classCount} classes`);
  ...
  if (allFunctions.length > 0) {
    ...
    for (const func of allFunctions.slice(0, 20)) {
      ...
      const summary = staticFunctionSummary(func);
  ...
  if (allClasses.length > 0) {
    ...
    for (const cls of allClasses) {
      ...
      if (cls.superClass) sections.push(`**Extends:** \`${cls.superClass}\`\n`);
      if (cls.implements.length > 0) sections.push(`**Implements:** ${cls.implements.map(i => `\`${i}\``).join(', ')}\n`);
  ...
  return {
    ...
    generator: 'modules',
    generatedAt: new Date(),
    inputHash: hashContent(module.files.map(f => f.contentHash).join(',')),
  };
}
