...
// ─── API Endpoints ───────────────────────────────────────────────────────────
function detectEndpoints(lines, filePath) {
    const endpoints = [];
...
function parseParams(str) {
    ...
    return str.split(',').map(p => {
        ...
        if (m)
            return { name: m[1], type: m[2]?.trim(), defaultValue: m[3]?.trim(), isOptional: !!m[3] };
...
function getDecorators(lines, funcLineIdx) {
    const decs = [];
...
function inferExports(functions, classes) {
    return [
        ...functions.filter(f => !f.name.startsWith('_')).map(f => ({ name: f.name, type: 'function', isDefault: false })),
        ...classes.filter(c => !c.name.startsWith('_')).map(c => ({ name: c.name, type: 'class', isDefault: false })),
    ];
}
